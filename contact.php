<?php include "inc/header.php"; ?>

	<?php 
		if($_SERVER['REQUEST_METHOD']=='POST'){
			$fname = $fm->validation($_POST['firstname']);
			$lname = $fm->validation($_POST['lastname']);
			$email = $fm->validation($_POST['email']);
			$body = $fm->validation($_POST['body']);
			
			$fname = mysqli_real_escape_string($db->link, $fname);
			$lname = mysqli_real_escape_string($db->link, $lname);
			$email = mysqli_real_escape_string($db->link, $email);
			$body = mysqli_real_escape_string($db->link, $body);
			
			$errorf = "";
			$errorl = "";
			$errore = "";
			$errorb = "";
			$msg = "";
			if(empty($fname)){
				$errorf = "Firstname must not be empty..";
			}if(empty($lname)){
				$errorl = "Lastname must not be empty..";
			}if(empty($email)){
				$errore = "invalid email address...";
			}if(empty($body)){
				$errorb = "msg field not be empty..";
			
					/* 
					if(empty($fname)){
						$error = "Firstname must not be empty..";
					}elseif(empty($lname)){
						$error = "Lastname must not be empty..";
					}elseif(empty($email)){
						$error = "email must not be empty..";
					}elseif(!filter_var($email, FILTER_VALIDATE_EMAIL)){
						$error = "invalid email address...";
					}elseif(empty($body)){
						$error = "msg field not be empty..";
						*/
			}else{
					$query = "INSERT INTO tbl_contact(firstname, lastname, email, body) 
					VALUES('$fname', '$lname', '$email', '$body')";
					$inserted_rows = $db->insert($query);
					
					if($inserted_rows){
						$msg = "msg sent successfully..";
					}else{
						$error = "msg not sent..";
					}
			}
		}
	?>	
<style>
	.cuserr{color:red; float:left;}
</style>
	<div class="contentsection contemplete clear">
		<div class="maincontent clear">
			<div class="about">
				<h2>Contact us</h2>
			<?php 
			/*
				if(isset($error)){
					echo "<span style='color:red'>$error</span>";
				}
			*/	
				if(isset($msg)){
					echo "<span style='color:green'>$msg</span>";
				}

			?>
			<form action="" method="post">
				<table>
				<tr>
					<td>Your First Name:</td>
					<td></td>
					<td>
						<?php 
							if(isset($errorf)){
								echo "<span class='cuserr'>$errorf</span>";
							}
						?>
					<input type="text" name="firstname" placeholder="Enter first name"/>
					</td>
				</tr>
				<tr>
					<td>Your Last Name:</td>
					<td></td>
					<td>
						<?php 
							if(isset($errorl)){
								echo "<span class='cuserr'>$errorl</span>";
							}
						?>
					<input type="text" name="lastname" placeholder="Enter Last name" />
					</td>
				</tr>
				
				<tr>
					<td>Your Email Address:</td>
					<td></td>
					<td>
						<?php 
							if(isset($errore)){
								echo "<span class='cuserr'>$errore</span>";
							}
						?>
					<input type="text" name="email" placeholder="Enter Email Address"/>
					</td>
				</tr>
				<tr>
					<td>Your Message:</td>
					<td></td>	
					<td>
						<?php 
							if(isset($errorb)){
								echo "<span class='cuserr'>$errorb</span>";
							}
						?>
					<textarea name="body"></textarea>
					</td>
				</tr>
				<tr>
					<td></td>
					<td>
					<input type="submit" name="submit" value="Send"/>
					</td>
				</tr>
		</table>
	<form>				
 </div>

</div>

<?php include "inc/sidebar.php"; ?>
<?php include "inc/footer.php"; ?>